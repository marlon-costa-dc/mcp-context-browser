...
fn create_test_directory() -> TempDir {
    ...
    fs::write(temp_dir.path().join("file1.rs"), "fn main() {}").expect("Failed to write file1");
    fs::write(temp_dir.path().join("file2.rs"), "fn test() {}").expect("Failed to write file2");
    fs::create_dir(temp_dir.path().join("subdir")).expect("Failed to create subdir");
    fs::write(temp_dir.path().join("subdir/file3.rs"), "mod test;").expect("Failed to write file3");

    temp_dir
}

#[tokio::test]
async fn test_sync_detects_new_files() {
...
#[tokio::test]
async fn test_sync_detects_file_modifications() {
...
#[tokio::test]
async fn test_sync_tracks_modification_times() {
...
#[tokio::test]
async fn test_sync_returns_changed_files() {
...
#[tokio::test]
async fn test_sync_handles_empty_directory() {
...
#[tokio::test]
async fn test_sync_filters_by_extension() {
    // Given: A directory with mixed file types
    let temp_dir = TempDir::new().expect("Failed to create temp directory");
    fs::write(temp_dir.path().join("code.rs"), "fn main() {}").expect("write");
    fs::write(temp_dir.path().join("readme.md"), "# Readme").expect("write");
    fs::write(temp_dir.path().join("data.json"), "{}").expect("write");
