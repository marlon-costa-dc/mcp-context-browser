...
/// Convert a SearchResult to a CodeChunk
///
/// Since SearchResult doesn't have metadata in mcb-domain, we construct
/// minimal metadata from available fields.
fn search_result_to_chunk(result: SearchResult, end_line_override: Option<u32>) -> CodeChunk {
    let end_line = end_line_override.unwrap_or(result.start_line);
...
///
/// Implements [`ChunkRepository`] using a vector store for persistent storage
/// and an embedding provider for generating semantic embeddings of code chunks.
...
impl ChunkRepository for VectorStoreChunkRepository {
    ...
    async fn find_by_id(&self, collection: &str, id: &str) -> Result<Option<CodeChunk>> {
        ...
        if let Some(result) = results.into_iter().next() {
            // Convert SearchResult to CodeChunk, using start_line as fallback for end_line
            let chunk = search_result_to_chunk(result, None);
    ...
    async fn find_by_collection(&self, collection: &str, limit: usize) -> Result<Vec<CodeChunk>> {
        ...
        let chunks: Vec<CodeChunk> = results
            .into_iter()
            .map(|result| search_result_to_chunk(result, None))
            .collect();
