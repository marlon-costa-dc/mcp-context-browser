...
impl BackupManager {
    ...
    pub async fn create_backup(&self, path: &str) -> Result<BackupInfo> {
        ...
        rx.await.unwrap_or_else(|_| Err(Error::internal("Backup response channel closed")))
    }
...
mod tests {
    ...
    async fn test_backup_creation() {
        ...
        let result = manager
            .create_backup(backup_path.to_str().unwrap())
            .await;
    ...
    async fn test_list_backups() {
        ...
        manager.create_backup(backup_path.to_str().unwrap()).await.unwrap();
