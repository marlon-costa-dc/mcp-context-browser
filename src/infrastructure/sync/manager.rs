...
use crate::domain::ports::sync::{SyncCoordinator, SyncOptions, SyncResult};
...
impl SyncProvider for SyncManager {
    async fn should_debounce(&self, codebase_path: &Path) -> Result<bool> {
        SyncManager::should_debounce(self, codebase_path).await
    }

    async fn update_last_sync(&self, codebase_path: &Path) {
        SyncManager::update_last_sync(self, codebase_path).await
    }

    async fn acquire_sync_slot(&self, codebase_path: &Path) -> Result<Option<SyncBatch>> {
        SyncManager::acquire_sync_slot(self, codebase_path).await
    }

    async fn release_sync_slot(&self, codebase_path: &Path, batch: SyncBatch) -> Result<()> {
        SyncManager::release_sync_slot(self, codebase_path, batch).await
    }

    async fn get_changed_files(&self, codebase_path: &Path) -> Result<Vec<String>> {
        SyncManager::get_changed_files(self, codebase_path).await
    }

    fn sync_interval(&self) -> Duration {
        SyncManager::sync_interval(self)
    }

    fn debounce_interval(&self) -> Duration {
        SyncManager::debounce_interval(self)
    }
}

#[async_trait]
impl SyncCoordinator for SyncManager {
    async fn should_debounce(&self, codebase_path: &Path) -> Result<bool> {
        SyncManager::should_debounce(self, codebase_path).await
    }

    async fn sync(&self, codebase_path: &Path, options: SyncOptions) -> Result<SyncResult> {
        // Check debounce unless force flag is set
        if !options.force && self.debounce.should_debounce(codebase_path) {
            return Ok(SyncResult::skipped());
        }

        // Perform the sync using existing sync_codebase method
        let synced = SyncManager::sync_codebase(self, codebase_path).await?;
        ...
        let changed_files = SyncManager::scan_for_changes(self, codebase_path).await;
