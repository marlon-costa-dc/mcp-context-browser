...
///
/// Implementations typically provide storage-specific logic while following
/// the trait contract for namespace isolation and TTL handling.
///
/// See [`NullCacheProvider`] for a minimal implementation example.
#[async_trait]
...
///
/// Use this type alias in function signatures to accept any cache implementation:
///
/// ```rust
/// use std::sync::Arc;
/// use mcp_context_browser::infrastructure::cache::SharedCacheProvider;
///
/// fn accepts_any_cache(cache: SharedCacheProvider) {
///     // Works with Moka, Redis, or Null cache providers
///     let backend = cache.backend_type();
///     println!("Using cache backend: {}", backend);
/// }
/// ```
