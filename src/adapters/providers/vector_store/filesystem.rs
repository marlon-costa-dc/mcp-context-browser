...
impl FilesystemVectorStore {
    ...
    pub async fn new(config: FilesystemVectorStoreConfig) -> Result<Self> {
        ...
        tokio::fs::create_dir_all(&config.base_path)
            .await
            .map_err(|e| {
                crate::domain::error::Error::io(format!("Failed to create base directory: {}", e))
            })?;
    ...
    async fn load_collection_state(&self, collection: &str) -> Result<()> {
        ...
        if shards_path.exists() {
            let mut entries = tokio::fs::read_dir(&shards_path).await.map_err(|e| {
                crate::domain::error::Error::io(format!("Failed to read shards directory: {}", e))
            })?;

            while let Some(entry) = entries.next_entry().await.map_err(|e| {
                crate::domain::error::Error::io(format!("Failed to read directory entry: {}", e))
            })? {
                let path = entry.path();
    ...
    async fn write_vector_to_shard(
        ...
        let metadata_bytes = serde_json::to_vec(metadata).map_err(|e| {
            crate::domain::error::Error::internal(format!("Failed to serialize metadata: {}", e))
        })?;
        ...
        .await
        .map_err(|e| crate::domain::error::Error::internal(format!("Blocking task failed: {}", e)))?
        .map_err(|e| crate::domain::error::Error::io(format!("Failed to write to shard: {}", e)))?;
    ...
    async fn read_vector_from_shard(
        ...
        .map_err(|e: std::io::Error| {
            crate::domain::error::Error::io(format!("Failed to read from shard: {}", e))
        })
    }
    ...
    async fn delete_collection(&self, name: &str) -> Result<()> {
        ...
        if index_path.exists() {
            tokio::fs::remove_file(index_path).await.map_err(|e| {
                crate::domain::error::Error::io(format!("Failed to delete collection index: {}", e))
            })?;
        }
